...
CREATE DEFINER=`root`@`localhost` PROCEDURE `delete_transaction`(
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `get_single_transaction`(IN `input_id` int)
BEGIN
    SELECT 
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `update_transaction_comment`(
    IN transaction_id INT,
